...
### Persistence & ordering (latest)
- Mode A persistence: user messages saved at request start; assistant messages saved on stream finish (appendResponseMessages + saveChat); server consumes stream to guarantee finish handler runs.
- Mode B persistence: assistant message saved synchronously (intro + resolved `MovieData[]` in `toolResults`).
- Message ordering: loader now sorts by `createdAt` ascending, fixing post-refresh jumbling.
- Safety: updates never modify `toolResults` (posters) — only set on insert — so Mode A replies can’t wipe Mode B posters.
...
- why it's right for you — source: planner output per pick (`reason`) — REMAINING: expose in Mode B response alongside each pick (preserve order)
...
- For "why it's right for you," ensure the Mode B server response includes per-pick `{ title, year, reason }` (either embed `reason` on each resolved pick or return a parallel `plannedPicks` array preserving order). UI can map by index or title.
- Keep the modal resilient: render what’s available; hide or label unavailable sections after ~5s.

Status for modal fields:
- Title, year, synopsis, poster: READY (from `MovieData`) — year via `release_date.slice(0,4)`.
- Ratings, providers, duration, genres, director, trailer: READY via enrichment hook.
- Why-it’s-right: NOT EXPOSED YET — add to Mode B response.

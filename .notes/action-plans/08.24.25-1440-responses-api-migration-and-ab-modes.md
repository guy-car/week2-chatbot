...
## 8. Route A Optimization Status (Current)
**Status: OPTIMIZED ENOUGH FOR NOW**

**Completed Optimizations:**
- ✅ **Model Selection**: Switched from `gpt-5` to `gpt-4o-mini-2025-04-14` for `decide_mode`
- ✅ **Output Simplification**: Changed from JSON schema to plain "A"/"B" output with token limits
- ✅ **Latency Reduction**: `decide_mode` now ~1.5s vs previous ~5s
- ✅ **System Prompt**: Added Watch Genie personality with clear conversational guidelines
- ✅ **Enhanced Logging**: Detailed timing summaries with response previews for both streaming/non-streaming
- ✅ **Streaming Support**: Implemented streaming for Mode A with fallback to non-streaming

**Current Performance:**
- Mode A ("hi"): ~2-3s total (acceptable for conversational flow)
- Mode B (recommendations): ~12s total (parallel TMDB lookups implemented)
- `decide_mode`: ~1.5s (optimized with smaller model)

**Next Phase Focus:**
- Mode B response contract (include reasons/years)
- Unit testing with SDK mocks
- New chat autostart recommendations

...
- **Action 1.1:** Create new route `src/app/api/chat-v2/route.ts` using OpenAI Responses API (`gpt-5`). Implement streaming that mirrors current client consumption.
- **Verification 1.1:** Send a simple user prompt; confirm streamed assistant text arrives and is persisted. No tools defined yet.

- **Action 1.2:** Add logging (prefix `[FLOW]`) for request start/end, tokens estimate, and final message save.
- **Verification 1.2:** Logs appear with timestamps for a sample request.

### Part 2: Mode Decision Tool (Stage 2)

- **Action 2.1:** Define `decide_mode` as a function tool (Responses API) with output schema:
...
- **Action 3.1:** Define `plan_picks` tool with inputs (server-provided):
  - `exclude`: `Array<{ id_tmdb: number; title: string; year: number }>`
  - `userLists`: `{ watchlist: { id_tmdb:number; media_type:'movie'|'tv'; title:string; year:number }[]; seen: same }`
...
- **Action 3.4:** Deduplication enforcement:
  - Plan-time: ensure `plan_picks` input includes `exclude` (current chat) and `userLists` (watchlist/seen)—instruct to avoid.
  - Enforcement-time: when resolving, reject IDs already in exclude or `userLists`; drop failed picks.
- **Verification 3.4:** Attempts to re-recommend an already suggested ID are blocked.

### Part 4: Observability & Safety

- **Action 4.1:** Add structured logs with timing for: `decide_mode`, `plan_picks` inputs/outputs, lookups per pick, dedupe events, overall turn summary.
...
- **Action 5.1:** Add an environment flag (or query param) to switch between `/api/chat-tmdb-tool` and `/api/chat-v2` in the client.
- **Verification 5.1:** Toggling uses the legacy route with GPT‑4o or the new route with GPT‑5.
...
```ts
userLists: {
  watchlist: Array<{ id_tmdb: number; media_type: 'movie'|'tv'; title: string; year: number }>,
  seen: Array<{ id_tmdb: number; media_type: 'movie'|'tv'; title: string; year: number }>,
}
```
...
- Picks: max 3, include year, avoid items in `exclude`, `userLists.watchlist`, and `userLists.seen`.
